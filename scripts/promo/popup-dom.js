/*
 * The popup's markup, shared by the still-frame generator and the video
 * generator. Mirrors what popup.js builds at runtime, element for element, so
 * both the store screenshots and the demo video show the interface that ships
 * rather than a drawing of it.
 *
 * Callers pass favDataUri(host); everything else is self-contained.
 */
...
// Staged tab data. Real sites, because a list of "Example Tab 1" would tell a
// viewer nothing about what the extension is for.
const T = {
...
module.exports = function makeDom(favDataUri) {
  const bulk = (label) => `
  ...
  // tab: { host, title, url, muted, cross, paused, panel, id }
  const tabRow = (tab) => {
    ...
    return `
    <div class="tab-entry" role="listitem"${tab.id ? ` data-row="${tab.id}"` : ''}>
  ...
  const titleRow = (withGlobal) => `
    <div class="title-row">
      ...
      <div id="global-bulk" class="bulk-controls${withGlobal ? '' : ' hidden'}">
        <span class="bulk-controls__label">All</span>
        <button type="button" class="bulk-btn bulk-btn--mute">${SPEAKER_MUTED}</button>
        <button type="button" class="bulk-btn bulk-btn--unmute">${SPEAKER}</button>
      </div>
    </div>`;
  ...
  return {
    SPEAKER, SPEAKER_MUTED, TUNE, PAUSE, PLAY, PERSON, PENCIL, CLOSE, INDICATOR,
    T, t, bulk, groupHeader, tabRow, tabList, container, titleRow, profileHeader,
    otherProfiles,
  };
};
