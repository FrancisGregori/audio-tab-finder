...
Everything renders the real `popup.css` and the markup from
`scripts/promo/popup-dom.js`, so rerunning the build after any popup change
keeps the listing accurate instead of letting it drift into a picture of an
interface that no longer exists.

Stills — `./scripts/build-promo-assets.sh`:
...
Video — `./scripts/build-promo-video.sh`:

| Slot | File | Spec |
|---|---|---|
| Video (YouTube URL) | `promo/demo.mp4` | 1920x1080, 30fps, 27s, H.264, silent |

The store takes a YouTube link rather than a file: upload `demo.mp4` to
YouTube (unlisted works) and paste the URL into the Video field.

The video is silent by design. A demo of an extension whose whole point is
unwanted noise should not open with a music bed, and any track would need
licensing that a free extension does not want to carry. The captions do the
narrating.

Suggested YouTube title and description:

> **Audio Tab Finder — find, mute, pause and control any noisy Chrome tab**
>
> One click shows every tab playing sound, across all your Chrome profiles.
> Mute a tab, mute everything but one, set a tab's volume from 0 to 100%, or
> pause playback outright — without hunting through your tabs.
>
> Free and open source: https://github.com/FrancisGregori/audio-tab-finder
